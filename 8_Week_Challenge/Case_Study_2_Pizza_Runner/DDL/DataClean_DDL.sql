...
WITH nulling AS (
   SELECT
    order_id,
    runner_id,
    CASE WHEN TRIM(LOWER(pickup_time)) IN ('null','nan','')
        THEN NULL
        ELSE pickup_time
    END AS pickup_time,
    CASE WHEN TRIM(LOWER(distance)) IN ('null','nan','')
        THEN NULL
        ELSE distance
    END AS distance,
    CASE WHEN TRIM(LOWER(duration)) IN ('null','nan','')
        THEN NULL
        ELSE duration
    END AS duration,
    CASE WHEN TRIM(LOWER(cancellation)) IN ('null','nan','')
        THEN NULL
        ELSE cancellation
    END AS cancellation
FROM runner_orders
